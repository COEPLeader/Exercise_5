...
```{r Problem 1.2}
  lettuce_harvest <- garden_harvest %>% 
    ...
    ggplotly(lettuce_graph, tooltip = "group")
```
...
```{r}
  small_trains %>% 
>>>>>>> e4e7dc60c4851679ead11bc9ecf7f2389f7e7dc3
    select(-c(delay_cause, delayed_number)) %>% 
    arrange(month) %>% 
    filter(departure_station %in% c("PARIS NORD", "PARIS MONTPARNASSE", "PARIS EST", "PARIS LYON")) %>% 
    mutate(month = month(month, label = TRUE)) %>% 
    unique() %>% 
    ggplot(aes(y = arrival_station, x = departure_station)) +
...
<<<<<<< HEAD
  ...
  animate(delay_graph, nframes = 100, duration = 10)
...
I wasn't able to make adjustments to the size of the graph in the usual way when animating. How do I do that?
=======
    theme(legend.position = "none") +
    transition_states(month) +
    labs()
